...
class Command(BaseCommand):
    ...
    def _compound_weakness_resistance(self, pokemon):
        ...
        compound_weakness = self._compound_type_effectivness({
            'primary_type': primary_type.weak,
            'secondary_type': secondary_type.weak if secondary_type else []
        })
        compound_resistance = self._compound_type_effectivness({
    ...
    def _compound_type_effectivness(self, type_effectivness, compound_effectivness = {}):
        compound_effectivness = compound_effectivness if compound_effectivness else {}

        for values in type_effectivness['primary_type']:
            if values[0] not in compound_effectivness:
                compound_effectivness.update({
                    values[0]: Decimal(values[1])
                })
            else:
                compound_effectivness[values[0]] = (
                    Decimal(compound_effectivness[values[0]]) * Decimal(values[1])
                )

        for values in type_effectivness['secondary_type']:
            if values[0] not in compound_effectivness:
                compound_effectivness.update({
                    values[0]: Decimal(values[1])
                })
        ...
        return compound_effectivness

    def _compound_effectivness(self, data, x, y):
        data[x] = data[x] * y
